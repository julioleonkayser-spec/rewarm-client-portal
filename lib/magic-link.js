...
async function sendMagicLinkEmail(email, magicUrl) {
  ...
  if (!apiKey) {
    if (process.env.NODE_ENV !== 'production') {
      console.log('[magic-link] DEV — RESEND_API_KEY not set. Sign-in link:', magicUrl);
      return;
    }
    console.error('[magic-link] FATAL — RESEND_API_KEY is not set. Add it in Vercel → Settings → Environment Variables.');
  ...
  console.log(`[magic-link] Sending sign-in email to ${email} from ${from}`);
  ...
  if (!response.ok) {
    ...
    console.error(`[magic-link] Resend rejected — status ${response.status}, from "${from}", to "${email}": ${detail}`);
    if (response.status === 403 || response.status === 422) {
      console.error('[magic-link] HINT: sender domain is likely not verified in Resend. Verify it at resend.com/domains or set RESEND_FROM_EMAIL to your verified address.');
    }
    throw new Error(`Resend ${response.status}: ${detail}`);
  }

  console.log(`[magic-link] Email delivered to ${email}`);
}
